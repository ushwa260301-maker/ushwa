...
/**
 * 근사 표기 — `"3월경"` · `"3월 말"` · `"3월 중순"`.
 *
 * 이런 꼬리표는 **날짜**를 흐릴 뿐 **월**을 흐리지 않는다. `"3월경"` 은 3월이라고
 * 말하고 있고, 우리가 저장하는 단위는 월이다. 그래서 받아들인다 —
 * 몇 월인지 아예 말하지 않는 `"초여름"` 과는 다른 경우다.
 */
const ABOUT = "(?:경|쯤|께|초|초순|중|중순|말|말경|상순|하순)";
/** 월 하나. `"6"` · `"6월"` · `"6월경"` — 꼬리표는 월 뒤에만 붙는다. */
const MONTH = `(\\d{1,2})\\s*(?:월\\s*${ABOUT}?)?`;
/** `"6~8월"` · `"6월~8월"` · `"6-8"` · `"3월 말~4월 초"` */
const MONTH_RANGE = new RegExp(`^${MONTH}\\s*[~∼〜\\-–—]\\s*${MONTH}$`);
const MONTH_ONE   = new RegExp(`^${MONTH}$`);
 ...
 *
 * **월을 말하지 않은 표기는 버린다.** `"봄"` · `"연중"` · `"초여름"` 은 몇 월인지
 * 원문에 없다 — 그럴듯한 달을 채우는 건 원본에 없는 데이터를 만드는 일이다.
 * 기준은 "불확실한가" 가 아니라 **"월이 적혀 있는가"** 다.
 *
 * 토큰 전체가 해석돼야 받는다. `"6월경 또는 이듬해"` 처럼 설명이 더 붙으면
 * 버린다 — 그 뒷말이 무엇을 한정하는지 알 수 없다.
 */
