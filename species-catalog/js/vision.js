...
// 합계 표식은 `합계` · `총계` 만 막고 있어 `총 합 계` 형태를 놓쳤다.
// 실환경 inv-063 의 `총     합     계     1,456,000` 은 공백 정규화 후
// `총합계` 가 되는데, `총\s*계` 는 총 다음에 계가 와야 하고 `합\s*계` 는
// `^` 앵커라 둘 다 매치되지 않아 품목 행이 됐다
// ({name:"총합계", price:1456000} → 저장 시 species 로 생성될 경로).
// `총?\s*합\s*계` 로 합계·총합계를 함께 덮고, 누락돼 있던 `총액` 도 넣는다.
const META_LINE_RE = /^(?:금\s*액|아\s*래|위\s*와|계\s*산|일\s*금|총?\s*합\s*계|총\s*계|총\s*액|공\s*급\s*자|공\s*급\s*받)/;
